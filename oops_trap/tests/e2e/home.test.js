...
test.describe('Аутентификация', () => {
  ...
  test('должна открывать модальное окно регистрации', async ({ page }) => {
    ...
    await expect(
      page.getByText('Confirm Password', { exact: true })
    ).toBeVisible()
  ...
  test('должна показывать ошибку при несовпадении паролей', async ({
    page
  }) => {
    await openSignUpModal(page)
    ...
    await expect(
      page.locator('.auth-form__error:has-text("Passwords don\'t match")')
  ...
  test('должна показывать успех при совпадении паролей', async ({ page }) => {
    ...
    await expect(
      page.locator('.auth-form__success:has-text("Passwords match")')
    ).toBeVisible({ timeout: 5000 })
  ...
  test('должна успешно регистрировать пользователя (с моком API)', async ({
    page
  }) => {
  ...
  test('должна успешно авторизовывать пользователя (с моком API)', async ({
    page
  }) => {
