...
vi.mock('@/components/game/traps/registry', () => ({
  TRAPS_BY_MAP: {
    ...
    map2: [{ id: 'trap3', name: 'arrows', component: 'TrapArrows' }]
  }
}))
...
describe('GameScreen Component', () => {
  ...
  beforeEach(() => {
    ...
    mockFetch.mockResolvedValue({
      json: () =>
        Promise.resolve({
          polygons: [{ id: 1, points: [0, 0, 100, 0, 100, 100] }]
        })
    })
  ...
  describe('Рендеринг компонентов в зависимости от роли', () => {
    ...
    it('НЕ должен рендерить CurrentPlayer для мафии', async () => {
      ...
      await nextTick()
      expect(wrapper.findComponent({ name: 'CurrentPlayer' }).exists()).toBe(
        false
      )
    })
    ...
    it('НЕ должен рендерить TrapController для не-мафии', () => {
      ...
      expect(wrapper.findComponent({ name: 'TrapController' }).exists()).toBe(
        false
      )
    })
  })
