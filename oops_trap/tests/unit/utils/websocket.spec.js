...
describe('createGameSocket', () => {
    ...
    test('должен создавать WebSocket с ws протоколом для http', () => {
        ...
        expect(mockWebSocket).toHaveBeenCalledWith('ws://localhost/ws/game/test-lobby-123');
        expect(console.log).toHaveBeenCalledWith('ws://localhost/ws/game/test-lobby-123');
    });
    ...
    test('должен создавать WebSocket с wss протоколом для https', () => {
        ...
        // Assert
        expect(mockWebSocket).toHaveBeenCalledTimes(1);
        expect(mockWebSocket).toHaveBeenCalledWith('wss://localhost/ws/game/secure-lobby-456');
        expect(console.log).toHaveBeenCalledWith('wss://localhost/ws/game/secure-lobby-456');
    });
    ...
    test('должен использовать VITE_SERVER_IP2 если он задан как пустая строка', () => {
        ...
        // Assert
        expect(mockWebSocket).toHaveBeenCalledTimes(1);
        expect(mockWebSocket).toHaveBeenCalledWith('ws://localhost/ws/game/empty-host-lobby');
        expect(console.log).toHaveBeenCalledWith('ws://localhost/ws/game/empty-host-lobby');
    });
    ...
    test('должен корректно работать с различными форматами lobbyId', () => {
        ...
        const testCases = [
            ...
            { lobbyId: 'abc-def-ghi', expected: 'ws://localhost/ws/game/abc-def-ghi' },
            { lobbyId: 'lobby_with_underscore', expected: 'ws://localhost/ws/game/lobby_with_underscore' },
            { lobbyId: 'LOBBY-UPPERCASE', expected: 'ws://localhost/ws/game/LOBBY-UPPERCASE' },
    ...
    test('должен логировать URL даже если WebSocket бросает исключение', () => {
        ...
        expect(console.log).toHaveBeenCalledWith('ws://localhost/ws/game/error-lobby');
    });
});